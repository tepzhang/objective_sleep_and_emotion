...
df.eda_change_trial <- df.eda %>% 
  mutate(event = case_when(time_segment <= 2 ~ "scl_baseline",
                           time_segment <=8 & time_segment > 2 ~ "scl_picture",
                           time_segment >8 ~ "scl_rating")) %>% 
...
df.physio_trial_matched_filt <- df.physio_trial_matched %>% 
  group_by(participant) %>% 
...
ggplot(df.physio_trial_matched_filt, aes(participant, emg_corr_change_pic))+
  geom_point(alpha = .2, 
             shape = 20, 
             # size = 2,
             color = "blue")+
...
```{r correlation between Corr_EMG and rating}
df.emg_rating_correlation <- df.emg_trial_matched_filt %>% 
  group_by(participant) %>% 
  summarise(cor_emg_raw = cor(ArousalRating.response, corr_picture, 
                              use = "na.or.complete"),
            cor_emg_change = cor(ArousalRating.response, emg_corr_change_pic, 
                                 use = "na.or.complete"))
...
ggplot(df.emg_trial_matched_filt, 
       aes(ArousalRating.response, corr_picture, color = participant))+
  geom_point(alpha = .2, size = 1,
             position = position_jitter(width = .2, height = 0))+
  geom_smooth(method = "lm", se = F, 
              alpha = .2, size = 1)+
  geom_text(data = df.emg_rating_correlation, 
            aes(x = 2, y = 40, label = sprintf("r=%0.2f", cor_emg_raw)))+
  facet_wrap(.~participant)+
  labs(title = sprintf("Raw corr EMG and arousal rating (average r = %.2f)", 
       mean(df.emg_rating_correlation$cor_emg_raw, na.rm = T)))+
  theme(legend.position = "none")
ggsave("plots/emg_rating_trial_level.png", width = 12, height = 9)
...
ggplot(df.emg_trial_matched_filt, 
  ...
  geom_smooth(method = "lm", se = F, 
              alpha = .2, size = 1)+
  geom_text(data = df.emg_rating_correlation, 
            aes(x = 2, y = 4, label = sprintf("r=%0.2f", cor_emg_raw)))+
  facet_wrap(.~participant)+
  labs(title = sprintf("Corr EMG change and arousal rating (average r = %.2f)", 
       mean(df.emg_rating_correlation$cor_emg_change, na.rm = T)))+
  theme(legend.position = "none")
ggsave("plots/emg_change_rating_trial_level.png", width = 12, height = 9)



# scatterplot at the individual level
ggplot(df.er_psg_emg, aes(watch_arou_rating, emg_corr_change_watch, color = intensity))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm", se = F)+
  facet_wrap(.~intensity)+
  labs(title = sprintf("Overall correlation: r = %.2f",
                       cor(df.er_psg_emg$watch_arou_rating,
                           df.er_psg_emg$emg_corr_change_watch, 
                           use = "na.or.complete")))+
  theme(legend.position = "none")
ggsave("plots/emg_change_rating_indi_level.png", width = 8, height = 6)

ggplot(df.er_psg_emg, aes(watch_arou_rating, emg_corr_raw_watch, color = intensity))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm", se = F)+
  facet_wrap(.~intensity)+
  labs(title = sprintf("Overall correlation: r = %.2f",
                       cor(df.er_psg_emg$watch_arou_rating,
                           df.er_psg_emg$emg_corr_raw_watch, 
                           use = "na.or.complete")))+
  theme(legend.position = "none")
ggsave("plots/emg_raw_rating_indi_level.png", width = 8, height = 6)
...
df.physio_trial_matched %>% 
  group_by(participant, task, intensity) %>% 
  summarise(scl_change = mean(scl_change_pic)) %>% 
  ggplot(aes(intensity, scl_change,
             color = participant))+
  geom_violin(color = "black",
              fill = "white")+
  geom_point(aes(group = participant), alpha = .3)+
  ...
  stat_summary(size = 1,
               ...
               geom = "pointrange")+
  facet_wrap(.~task)+
  labs(title = "SCL change from baseline")+
  theme(legend.position = "none")

ggsave("plots/scl_change_distribution.png", width = 8, height = 6)
```

```{r correlation between EDA, EMG, and rating}
...
df.emg_ind <- df.physiotrial_matched %>% 
  group_by(participant, task, intensity) %>% 
  summarise(emg_corr_change = mean(emg_corr_change_pic),
...
# write the individual-level emg data to the server
write_csv(df.er_psg_emg,
          "Z:/SBER/SBER1_physio_R56/data/Full_Study/session2_data/ier/IER_psg_emg_combined_participant_level.csv")
...
df.er_psg_emg <- read_csv("Z:/SBER/SBER1_physio_R56/data/Full_Study/session2_data/ier/IER_psg_emg_combined_participant_level.csv") %>% 
  mutate(participant = as.character(participant),
