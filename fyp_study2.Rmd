...
## other no end-recording-time cases
...
df.psg_summary <- df.psg_summary %>%
  filter(!pid %in% c("5029", "5675", "1046", "3694", "2001", "1070", "1570"))
...
mod_tst_lmer2 <- lmer(arou_rating ~ intensity_num*psg_tst + 
                        ...
                        (1|stimulus), 
                      data = df.er_trial_psg %>% filter(task == "watch"))
mod_tst_lmer2 %>% summary()

mod_tst_lmer2 %>% 
  augment() %>%
  clean_names() %>% 
  ggplot(aes(psg_tst, fitted, color = factor(intensity_num)))+
  geom_point(alpha = .2)+
  geom_smooth(method = "lm", color = "black")+
  facet_wrap(.~intensity_num)+
  theme(legend.position = "none")
