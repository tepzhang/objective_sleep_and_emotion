...
ggsave("rem-reactivity correlation.pdf", width = 12, height = 9)

#### REM and regulation ####
p_rem5 <- ggplot(df.er_psg %>% filter(psg_rem >0),
       aes(psg_rem, reappraise_val_change, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "REM and reappraisal (REM > 0)")

p_rem6 <- ggplot(df.er_psg %>% filter(psg_rem >0),
       aes(psg_rem, distract_val_change, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "REM and distraction (REM > 0)")

plot_grid(p_rem5, p_rem6)
ggsave("rem-regulation correlation.pdf", width = 12, height = 5)
```

```{r visualization 3 SWS}
## SWS ##
p_sws1 <- ggplot(df.er_psg ,
       aes(psg_n3, watch_val_rating, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "SWS and reactivity (all cases)")

p_sws2 <- ggplot(df.er_psg %>% filter(psg_n3 > 0),
       aes(psg_n3, watch_val_rating, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "SWS and reactivity (SWS>0)")


```

```{r visualization 4 TST}
# TST (total sleep time)
p_tst1 <- ggplot(df.er_psg,
       aes(psg_tst, watch_val_rating, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "Total sleep time and reactivity (all cases)")

p_tst2 <- ggplot(df.er_psg,
       aes(psg_tst, reappraise_val_change, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "Total sleep time and reappraisal")

p_tst3 <- ggplot(df.er_psg,
       aes(psg_tst, distract_val_change, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "Total sleep time and distraction")
```


```{r analysis: PSG and ER}
# REM duration
