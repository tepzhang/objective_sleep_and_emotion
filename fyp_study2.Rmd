...
df.emg <- df.emg %>% 
  ...
  mutate(participant = factor(participant),
         task = factor(task, levels = c("Watch", "Rethink", "Distract")))
...
```{r process EMG data}
# calculate the percentage change compared to the baseline (first 3 seconds)
df.emg_trial <- df.emg %>% 
  mutate(picture_on = ifelse(time_segment > 3, "corr_picture", "corr_baseline")) %>% 
  group_by(participant, trial, task, picture_on) %>% 
  summarise(emg_corr = mean(emg_corr)) %>% 
  spread(picture_on, emg_corr) %>% 
  ungroup() %>% 
  mutate(emg_corr_change = (corr_picture - corr_baseline)/corr_baseline)

# remove participants who had less than 100 trials
df.emg_trial <- df.emg_trial %>% 
  group_by(participant) %>% 
  mutate(n_trial = n()) %>% 
  filter(n_trial > 100) %>% #1061, 1152, 5767 were filtered out
...
df.emg_trial %>% 
  group_by(participant) %>% 
  summarise(n_trial_emg = n()) %>% 
  left_join(agg_df %>% 
              group_by(participant) %>% 
              summarise(n_trial_task = n()),
            by = "participant") %>% 
  mutate(n_diff = n_trial_emg - n_trial_task)


# match the trials
df.emg_trial_matched <-  df.emg_trial %>% 
  left_join(agg_df %>% 
              select(participant, intensity, task) %>% 
              mutate(trial = row_number()) %>% 
              rename(task_er = task),
            by = c("participant", "trial")) %>% 
  mutate(match = task == task_er,
         intensity = factor(intensity, levels = c("ntr", "low", "mid", "high")),
         task = factor(task, levels = c("watch", "reappraise", "distract")))

# check the match
df.emg_trial_matched$match %>% unique() # all matched

# plot the distribution of trial_level emg_change
ggplot(df.emg_trial_matched, aes(participant, emg_corr_change))+
  geom_point(alpha = .2, 
             shape = 20, 
             # size = 2,
             color = "blue")+
...
df.emg_trial_matched_filt <- df.emg_trial_matched %>% 
  filter(participant != "5446") %>% #only noise
  group_by(participant) %>% 
  mutate(mean_change = mean(emg_corr_change),
         sd_change = sd(emg_corr_change),
  ...
  filter(emg_corr_change <= mean_change + 2.5*sd_change,
         emg_corr_change >= mean_change - 2.5*sd_change,
         corr_baseline <= mean_corr_baseline + 2.5*sd_corr_baseline,
         corr_baseline >= mean_corr_baseline - 2.5*sd_corr_baseline,
         corr_picture <= mean_corr_picture + 2.5*sd_corr_picture,
         corr_picture >= mean_corr_picture - 2.5*sd_corr_picture) %>% 
  ungroup()

ggplot(df.emg_trial_matched_filt, aes(participant, emg_corr_change))+
  geom_point(alpha = .2, 
             shape = 20, 
             # size = 2,
             color = "blue")+
...
df.emg_trial_matched_filt %>% 
  group_by(participant, task, intensity) %>% 
  summarise(emg_corr_change = mean(emg_corr_change)) %>% 
...
df.emg_ind <- df.emg_trial_matched_filt %>% 
  group_by(participant, task, intensity) %>% 
  summarise(emg_corr_change = mean(emg_corr_change)) %>% 
  spread(task, emg_corr_change) %>% 
  rename(emg_change_watch = watch,
         emg_change_reappraise = reappraise,
         emg_change_distract = distract)

# merge the individual-level emg data with the ER_psg data
df.er_psg_emg <- df.er_psg %>% 
  left_join(df.emg_ind, by = c("participant", "intensity"))

# check who had ER behavioral data but not emg data
list_no_emg <- (df.er_psg_emg %>% 
  filter(is.na(emg_change_watch)) %>% 
  select(participant) %>% 
  unique())$participant
...
lmerTest::lmer(scale(emg_change_watch) ~ scale(psg_tst)*scale(intensity_num) +
                   (1+scale(intensity_num)|participant), 
               data = df.er_psg_emg) %>% summary()
...
# visualize the data
df.er_psg_emg$intensity = 
  factor(df.er_psg_emg$intensity, levels = c("ntr", "low", "mid", "high"))
                              
ggplot(df.er_psg_emg,
       aes(psg_tst, emg_change_watch, color = intensity))+
  geom_point(alpha = .8)+
  facet_wrap(.~intensity, nrow = 1)+
  geom_smooth(method = "lm")+
  labs(title = "TST and EMG corrugator-reactivity")
ggsave("plots/tst-corrugator emg correlation.png", width = 8, height = 6)
...
p_rem_emg <- ggplot(df.er_psg_emg,
       aes(psg_rem, emg_change_watch, color = as.factor(intensity_num))) +
  geom_point(alpha = .3)+
  geom_smooth(method = "lm")+
  facet_wrap(.~intensity, nrow = 1)+
...
p_sws_emg <- ggplot(df.er_psg_emg,
       aes(psg_sws, emg_change_watch, color = as.factor(intensity_num))) +
  geom_point(alpha = .3)+
  geom_smooth(method = "lm")+
  facet_wrap(.~intensity, nrow = 1)+
