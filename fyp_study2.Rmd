...
```{r visualization: mean-center the raw rating}
...
ggplot(df.er_psg,
  ...
  geom_smooth(data = df.er_psg,
              ...
              color = "black",
              method = "lm", se = F)+
  labs(title = "Total sleep time and arousal-reactivity (all cases)")
...
lm(watch_val_rating ~ psg_tst + psg_rem_perc, 
               data = df.er_psg %>% filter(intensity == "high" , psg_rem > 0)) %>% 
  summary()
...
## interaction
mod_tst3 <- lm(watch_arou_rating ~ psg_tst*intensity_num, 
               data = df.er_psg %>% filter(psg_rem > 0))
summary(mod_tst3)

mod_tst4 <- lm(watch_arou_rating ~ psg_tst*intensity_num, 
               data = df.er_psg %>% filter(psg_rem > 0))
summary(mod_tst4)
```
