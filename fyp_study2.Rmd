...
```{r visualize PSG data}
p_hist1 <- hist(df.psg_summary$psg_tib, main = "PSG time in bed")
p_hist2 <- hist(df.psg_summary$psg_tst, main = "PSG total sleep time")
p_hist3 <- hist(df.psg_summary$psg_n3, main = "PSG SWS time")
p_hist4 <- hist(df.psg_summary$psg_rem, main = "PSG REM sleep time")

# ????????
grid.arrange(list(p_hist1,
             p_hist2,
             p_hist3,
             p_hist4))

# REM and SWS
ggplot(df.psg_summary,
       aes(psg_rem, psg_n3))+
  geom_point(alpha = .5)+
...
plot_grid(p_rem1, p_rem2, p_sws1, p_sws2, col =2)
...
ggplot(df.er_psg %>% filter(psg_tst > 0),
       aes(psg_tst, watch_val_rating, color = intensity))+
  geom_point(alpha = .4)+
  geom_smooth(method = "lm")+
  labs(title = "Total sleep time and reactivity (TST>0)")
```
