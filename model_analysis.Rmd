...
ggplot(df.all_reactivity, aes(psg_tst, emg_corr_change_watch))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm", se = F)+
  labs(title = sprintf("Correlation between PSG sleep \n and Corrugator EMG change: r = %.2f",
                       cor(df.all_reactivity$psg_tst,
                           df.all_reactivity$emg_corr_change_watch, 
                           use = "na.or.complete")))


ggplot(df.all_reactivity, aes(psqi_c1, emg_corr_change_watch))+
  geom_point(alpha = .5, position = position_jitter(height = 0, width = .15))+
  geom_smooth(method = "lm", se = F)+
  labs(title = sprintf("Correlation between PSQI sleep quality \n and Corrugator EMG change: r = %.2f",
                       cor(df.all_reactivity$psqi_c1,
                           df.all_reactivity$emg_corr_change_watch, 
                           use = "na.or.complete")))
```
...
ggplot(df.all_reactivity, aes(psg_tst, scl_change_pic_watch))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm", se = F)+
  labs(title = sprintf("Correlation between PSG sleep \n and SCL change: r = %.2f",
                       cor(df.all_reactivity$psg_tst,
                           df.all_reactivity$scl_change_pic_watch, 
                           use = "na.or.complete")))

cor.test(df.all_reactivity$psg_tst, df.all_reactivity$scl_change_pic_watch)
cor.test(df.all_reactivity$psg_tst, df.all_reactivity$scl_baseline_watch)
cor.test(df.all_reactivity$psg_tst, 
         df.all_reactivity$scl_baseline_watch+ df.all_reactivity$scl_change_pic_watch)


ggplot(df.all_reactivity, aes(psqi_c1, scl_change_pic_watch))+
  geom_point(alpha = .5, position = position_jitter(height = 0, width = .15))+
  geom_smooth(method = "lm", se = F)+
  labs(title = sprintf("Correlation between PSQI sleep quality \n and SCL change: r = %.2f",
                       cor(df.all_reactivity$psqi_c1,
