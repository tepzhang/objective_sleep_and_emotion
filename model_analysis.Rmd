...
# diary first night (with PSG)
df.diary_1st <- df.diary %>% 
  ...
  summarise(totalsleeptime = totalsleeptime_AM[1],
            subjsleepquality = subjsleepquality_AM_intgr[1],
            negativetoday = negativetoday_PM[2],
            emoarousaltoday = emoarousaltoday_PM[2]) %>% 
  ungroup() %>% 
  full_join(df.all_reactivity %>% select(participant, contains("psg_")),
            by = "participant")

ggpairs(df.diary_1st %>% 
          select(totalsleeptime, negativetoday, emoarousaltoday, 
                 psg_tst, psg_slp_eff, psg_waso),
        lower = list(continuous = wrap("smooth", alpha = 0.4, color = "blue")))
ggsave("plots/diary_fist_night_correlation.png", width = 8, height = 8)

# averaged individual level data
df.diary %>% glimpse()
...
# correlation matrix
GGally::ggpairs(df.diary_ind %>% 
                  select(totalsleeptime_diary,
                         sleep_efficiency_diary,
                         subjsleepquality_diary,
                         satisfiedsleep_diary,
                         emoarousaltoday_diary,
                         negativetoday_diary)) +
  theme_grey()
...
df.scl_baseline1_ER <-read_csv("Z:/SBER/SBER1_physio_R56/data/Full_Study/session2_data/ier/SCL_baseline_task_to_ER_task.csv") %>% 
  mutate(participant = as.character(participant))

df.all_reactivity <- df.all_reactivity %>% 
  left_join(df.er_date, by = "participant") %>% 
  left_join(df.psg_date, by = "participant") %>% 
  left_join(df.scl_baseline1_ER, by = "participant")
...
# write measurement dates
write_csv(df.all_reactivity %>% select(participant, contains("date")),
          "Z:/SBER/SBER1_physio_R56/data/Full_Study/measurement_dates.csv")
...
difftime(date_er, date_psg, units = "days") %>% as.numeric() %>% mean(na.rm = T)
...
```{r sleep and baseline_to_ER scl}
df.scl_sleep_corr <- data.frame(
  participant = rep(df.all_reactivity$participant, 7),
  reactivity = c(rep(df.all_reactivity$scl_change_bsl_to_er, 7)),
  sleep = rep(c(df.all_reactivity$psqi_sd,
                df.all_reactivity$psqi_ssq,
                df.all_reactivity$totalsleeptime_diary,
                df.all_reactivity$subjsleepquality_diary,
                df.all_reactivity$psg_tst,
                df.all_reactivity$psg_slp_eff,
                df.all_reactivity$psg_waso), 1),
  reactivity_measure = rep(c("BSL-ER SCL change"),
                           each = 7*nrow(df.all_reactivity)),
  sleep_measure = rep(rep(c("PSQI slp duration",
                            "PSQI slp quality", 
                            "diary slp duartion",
                            "diary slp quality",
                            "PSG slp duration",
                            "PSG slp efficiency",
                            "PSG WASO"),
                          each = nrow(df.all_reactivity)), 1)) %>% 
  mutate(sleep_measure = factor(sleep_measure, 
                                levels = c("PSQI slp duration",
                                           "PSQI slp quality", 
                                           "diary slp duartion",
                                           "diary slp quality",
                                           "PSG slp duration",
...
tb.scl_sleep_corr <- df.scl_sleep_corr %>% 
...
ggplot(df.scl_sleep_corr, aes(sleep, reactivity, color = sleep_measure))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm")+
  geom_text(data = tb.scl_sleep_corr,
            color = "black",
            aes(x = rep(c(4, 0.1, 350, 2.5, 5, .55, 1), 1), 
                y = rep(c(2), each = 7),
                label = sprintf("r=%0.2f", correlation)),
            hjust = 0, vjust = 0)+
  facet_grid(reactivity_measure ~ sleep_measure , scales = "free")+
  labs(y = "SCL change", x = "Sleep")+
  theme(legend.position = "none")

ggsave("plots/sleep_SCL_change_BSL_to_ER2.png", width = 12, height = 3)
```


```{r all sleep and reactivity}
df.reactivity_sleep_corr <- data.frame(
  participant = rep(df.all_reactivity$participant, 56),
  reactivity = c(rep(df.all_reactivity$pers_anr, 7),
                 rep(df.all_reactivity$negativetoday_diary, 7),
                 rep(df.all_reactivity$watch_val_rating, 7),
                 rep(df.all_reactivity$emg_corr_change_watch, 7),
                 rep(df.all_reactivity$watch_arou_rating, 7),
                 rep(df.all_reactivity$scl_change_pic_watch, 7),
                 rep(df.all_reactivity$sra_change_pic_watch, 7),
                 rep(df.all_reactivity$srr_change_pic_watch, 7)),
  sleep = rep(c(df.all_reactivity$psqi_sd,
                df.all_reactivity$psqi_ssq,
                df.all_reactivity$totalsleeptime_diary,
                df.all_reactivity$subjsleepquality_diary,
                df.all_reactivity$psg_tst,
                df.all_reactivity$psg_slp_eff,
                df.all_reactivity$psg_waso), 8),
  reactivity_measure = rep(c("PERS neg reactivity",
                             "Diary negative",
                             "Valence rating",
  ...
  sleep_measure = rep(rep(c("PSQI slp duration",
                            ...
                            "PSG slp efficiency",
                            "PSG WASO"),
                          each = nrow(df.all_reactivity)), 8)) %>% 
  mutate(reactivity_measure = factor(reactivity_measure, 
                                     levels = c( "PERS neg reactivity",
                                                 "Diary negative",
                                                 "Valence rating",
                                                 "Corrugator EMG change", 
                                                 "Arousal rating",
...
ggplot(df.reactivity_sleep_corr, aes(sleep, reactivity, color = sleep_measure))+
  ...
  geom_text(data = tb.reactivity_sleep_corr,
            color = "black",
            aes(x = rep(c(4, 0.1, 350, 2.5, 5, .55, 1), 8), 
                y = rep(c(4.1, 52, 4,.7,4, .1, .01, .04), each = 7),
                label = sprintf("r=%0.2f", correlation)),
            hjust = 0, vjust = 0)+
  facet_grid(reactivity_measure ~ sleep_measure , scales = "free")+
  labs(y = "Reactivity", x = "Sleep")+
  theme(legend.position = "none")
ggsave("plots/sleep_reactivity_all_measures.png", width = 12, height = 12)

```
```{r internal correlation among measures, warning=F}
library(GGally)

ggpairs(df.all_reactivity %>% 
          select(psqi_sd,
                 psqi_ssq,
                 totalsleeptime_diary,
                 subjsleepquality_diary,
                 psg_tst,
                 psg_slp_eff,
                 psg_waso),
        lower = list(continuous = wrap("smooth", alpha = 0.4, color = "blue")),
        columnLabels = c("PSQI slp duration",
                         "PSQI slp quality", 
                         "diary slp duartion",
                         "diary slp quality",
                         "PSG slp duration",
                         "PSG slp efficiency",
                         "PSG WASO"))
ggsave("plots/internal_correlation_sleep.png", width = 9, height = 9)
...
ggpairs(df.all_reactivity %>% 
          select(pers_anr,
                 negativetoday_diary,
                 watch_val_rating, 
                 emg_corr_change_watch,
                 watch_arou_rating, 
                 scl_change_pic_watch, 
                 sra_change_pic_watch,
                 srr_change_pic_watch),
        lower = list(continuous = wrap("smooth", alpha = 0.4, color = "blue")),
        columnLabels = c("PERS reactivity",
                         "Diary negative",
                         "Valence rating",
                         "Corr EMG change", 
                         "Arousal rating",
                         ...
                         "SRA change",
                         "SRR change"))
ggsave("plots/internal_correlation_reactivity.png", width = 9, height = 9)
```
