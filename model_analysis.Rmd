...
df.er_psg <- df.er_psg %>% 
  left_join(df.ind_diff %>% 
              select(partID, starts_with("psqi"), starts_with("pers")),
            by = c("participant" = "partID"))

# fyp final sample
# psych::corr.test(df.er_psg %>% 
#                    filter(intensity == "high") %>%
#                    select(psqi_sleep_hours, pers_gnr))
cor.test(df.er_psg$pers_gnr[df.er_psg$intensity=="high" & !is.na(df.er_psg$psg_tst)],
         df.er_psg$psqi_sleep_hours[df.er_psg$intensity=="high" & !is.na(df.er_psg$psg_tst)])


# R56 final sample
cor.test(df.ind_diff$pers_gnr, df.ind_diff$hourssleep_Hours)

# scatter plots
p_psqi_pers1 <- ggplot(df.er_psg %>% filter(intensity == "high", !is.na(psg_tst)),
       aes(psqi_sleep_hours, pers_gnr))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm")+
  labs(title = "Study2 sample (n = 62), r = -.13")

p_psqi_pers2 <-ggplot(df.ind_diff,
       aes(hourssleep_Hours, pers_gnr))+
  geom_point(alpha = .5)+
  geom_smooth(method = "lm")+
  labs(title = "R56 whole sample (n = 104), r = -.18")

plot_grid(p_psqi_pers1, p_psqi_pers2)
...
p_pers1 <- ggplot(df.er_psg,
       aes(pers_gnr, watch_val_rating, color = intensity))+
  geom_point(alpha = .8)+
  geom_smooth(method = "lm")+
  facet_wrap(.~intensity, nrow = 1)+
...
p_pers2 <- ggplot(df.er_psg,
       aes(pers_gnr, watch_arou_rating, color = intensity))+
  geom_point(alpha = .8)+
  geom_smooth(method = "lm")+
  facet_wrap(.~intensity, nrow = 1)+
...
cor.test(df.er_psg$pers_gnr[df.er_psg$intensity!="ntr" & !is.na(df.er_psg$psg_tst)],
